...
class Array2Xml
{
     ...
     * @throws \Exception
     */
    public static function &createXML($node_name, array $arr = array())
    {
     ...
     * @return \DOMNode
     */
    protected static function &convert($node_name, $arr = array())
    {
        $xml = static::getXMLRoot();
        ...
        if (is_array($arr)) {
            ...
            if (isset($arr['@comment']) && is_string($arr['@comment'])) {
                $node->appendChild($xml->createComment(static::bool2str($arr['@comment'])));
                unset($arr['@comment']);
            } else {
                if (isset($arr['@xml'])) {
